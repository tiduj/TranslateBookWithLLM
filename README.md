...
## Features
  ...
  - 📚 **EPUB Support**: Seamless translation of `.epub` book files, preserving their original structure.
...
pip install flask flask-cors flask-socketio python-socketio requests tqdm aiohttp lxml ebooklib

# Or install minimal dependencies for CLI only
pip install requests tqdm lxml ebooklib
```
...
```bash
# Basic English to French translation of a text file
python translate.py -i book.txt -o book_fr.txt

# Translate an EPUB file from English to French
python translate.py -i my_novel.epub -o my_novel_fr.epub -sl English -tl French

# English to German with a different model
...
### Content Preparation
  ...
  - Use plain text (`.txt`) format or standard EPUB (`.epub`) files.
